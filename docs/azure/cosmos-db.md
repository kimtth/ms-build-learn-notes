## Azure Cosmos DB Developer Study Notes
...
1. [Search Azure Cosmos DB for NoSQL data with Azure AI Search](https://learn.microsoft.com/en-us/training/modules/search-azure-cosmos-db-sql-api-data-azure-cognitive-search)
...
- Use **Azure Cosmos DB for NoSQL** as the name of the native JSON and SQL-query API. The older names *SQL API* and *Core (SQL) API* occur in older learning resources and SDK examples.
- Prefer Microsoft Entra ID authentication and native data-plane RBAC for new secure workloads. Key-based authentication can be disabled after applications and operational tooling are migrated. See [role-based access control for Azure Cosmos DB for NoSQL](https://learn.microsoft.com/en-us/azure/cosmos-db/nosql/security/how-to-grant-data-plane-role-based-access).
- Validate backup mode and recovery objectives explicitly. Continuous backup supports point-in-time restore, but network settings, data-plane role assignments, private endpoints, and stored procedures aren't restored automatically. See [continuous backup and point-in-time restore](https://learn.microsoft.com/en-us/azure/cosmos-db/continuous-backup-restore-introduction).
- Azure Cosmos DB for NoSQL is also a vector database. For AI applications, evaluate integrated vector and hybrid search only after validating partitioning, indexing, retrieval quality, latency, and cost at representative scale. Keep analytical workloads isolated through Fabric mirroring or Azure Synapse Link rather than overloading the transactional path.
...
- Azure Cosmos DB for NoSQL has a few advantages such as:
    1. Globally distributed data with configurable consistency, partitioning, throughput, and availability options. Verify the service-level agreement that applies to the selected account configuration.
    1. SDKs for popular languages, a native NoSQL API, and APIs for MongoDB, Cassandra, Gremlin, and Table. Select an API based on application compatibility and query needs.
    1. Fully managed capacity modes, including provisioned throughput, autoscale, and serverless. Choose by workload predictability, scale, and cost model.

        <img src="../../assets/images/2-nosql-db.png" alt="package" width="450"/>
    ...
    - Selecting a partition key path for a container can be one of the most important design decisions for a new workload. Review the choosing a partition key documentation for a deeper technical explanation and best practices.
        ...
        <img src="../../assets/images/3-resource-hierarchy.png" alt="package" width="450"/>
    ...
    - Containers can also store JavaScript based stored procedures, triggers and user-defined-functions (UDFs)
        ...
        <img src="../../assets/images/2-item.png" alt="package" width="550"/>

    - API Selection

        <img src="../../assets/images/3-select-api.png" alt="package" width="550"/>

    - When configuring Azure Cosmos DB, you can provision throughput at either or both the database and container levels.

        <img src="../../assets/images/2-mixed.png" alt="package" width="550"/>
    ...
    - RU: Request units are a rate-based currency. They are used to make it simple to talk about physical resources like memory, CPU, and IO when performing requests in Azure Cosmos DB. Request units are used to measure both foreground and background activities.
        ...
        <img src="../../assets/images/3-request-units.png" alt="package" width="350"/>
...
- For new applications, authenticate with Microsoft Entra ID, assign a data-plane role such as **Cosmos DB Built-in Data Contributor** at the narrowest practical scope, and reuse one `CosmosClient` for the lifetime of the application. Avoid creating a client per request or storing account keys in source code.
    ...
    Container container = client.GetContainer("appdb", "products");
    ```
...
- Enable local development with the **Azure Cosmos DB emulator** or the Linux emulator container where supported. Confirm the current emulator platform, API coverage, and feature limitations before depending on it for test automation; service features such as serverless, autoscale, and previews might not be represented locally.
...
- Bulk execution is a throughput optimization, not a latency optimization. Enable it only for high-volume concurrent operations, and keep the client singleton.
...
- To conceptualize our container’s index, we tend to think of the index as a union of all trees for each item in the container. Altogether, this creates an inverted index which gives our database engine something fast and efficient to traverse when performing query operations. Each node in the tree has metadata indicating which items in our index matches that specific node.
    ...
    <img src="../../assets/images/2-inverted-tree.png" alt="package" width="550"/>
    ...
    <img src="../../assets/images/2-search-tree-02.png" alt="package" width="550"/>
...
- To configure an **Azure Function to use an Azure Cosmos DB for NoSQL binding**, use managed identity and Microsoft Entra authentication where the binding and hosting configuration support it. Otherwise, store connection settings outside source control and plan a migration away from key-based authentication.
- **Azure AI Search can index data from Azure Cosmos DB for NoSQL.**
    - An Azure AI Search service is comprised of a few core components:
    ...
    1. Data Sources that connect Azure AI Search to various data platforms

    <img src="../../assets/images/2-indexers.png" alt="package" width="550"/>
...
- ER Diagram
    ...
    <img src="../../assets/images/1-full-relational-model.png" alt="package" width="550"/>
    ...
    <img src="../../assets/images/6-relationship-between-physical-and-logical-partitions.png" alt="package" width="550"/>
...
- Design a data partitioning strategy
    - The Product table need to optimize, it has the many-to-many relationship your product table has with product tags.
        ...
        <img src="../../assets/images/2-product-denormalized.png" alt="package" width="200"/>
    ...
    1. Referential integrity:  However, when data is denormalized like this, you need a way to maintain referential integrity between the master data in the productCategory and productTag containers, and the product container.
        ...
        <img src="../../assets/images/4-change-feed.png" alt="package" width="200"/>
    ...
    1. Combine multiple entities in the same container: The customer container has a partition key of ID and salesOrder has a partition key of customerId. When data share a partition key and have similar access patterns, they're candidates for being stored in the same container. As a NoSQL database, Azure Cosmos DB is schema agnostic, so mixing entities with different schema is not only possible but, under these conditions, it's also another best practice. But to combine the data from these two containers, you'll need to make more changes to your schema.
        ...
        <img src="../../assets/images/6-sales-orders-customers.png" alt="package" width="400"/>
        ...
        <img src="../../assets/images/6-sales-orders-customers-query.png" alt="package" width="400"/>
    ...
    1. Denormalizing aggregates: Before your new model is complete, one last operation to look at is to query your top 10 customers by the number of sales orders. In your current model, you first do a group by on each customer and sum for sales orders in your customer container. You then sort in descending order and take the top 10 results. Even though customers and sales orders sit in the same container, this type of query is not something you can currently do.
        ...
        <img src="../../assets/images/7-sales-order-count.png" alt="package" width="400"/>
    ...
    1. Here then is your final design. With the merging of the product categories and tags, you've gone from nine relational tables to just three containers.
        ...
        <img src="../../assets/images/9-design-final.png" alt="package" width="400"/>
...
- Enable integrated cache
    ...
    <img src="../../assets/images/3-dedicated-gateway-config.png" alt="package" width="550"/>
...
- Cosmos DB monitors its server-side counters using:
    ...
    <img src="../../assets/images/2-monitor-cosmos-db.png" alt="package" width="550"/>
...
- Backup
    1. Azure Cosmos DB offers periodic and continuous backup modes. Choose the mode during account planning based on the required restore experience, retention window, recovery point objective, and cost.
    1. Continuous backup enables point-in-time restore into a target account or, in supported scenarios, an existing account. Restore is not a complete account-configuration clone: reapply private endpoints, firewall rules, data-plane RBAC assignments, customer-managed keys, diagnostic settings, and other settings after restore.
    1. Confirm the most recent restorable timestamp before a recovery operation and test the recovery runbook. Retention and supported restore scenarios vary by selected continuous-backup tier.
- Security
    ...
    |Microsoft Entra integration and Azure RBAC    |Use Azure RBAC for control-plane management and native Azure Cosmos DB data-plane RBAC for data access. Keep these planes separate and assign least-privilege roles to Microsoft Entra identities.              |
...
- When creating Bicep files or Azure Resource Manager templates (ARM templates), you need to understand what resource types are available, and what values to use in your template.
  ...
  <img src="../../assets/images/2-hierarchy.png" alt="package" width="550"/>
