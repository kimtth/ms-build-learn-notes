...
## Security, Compliance, and Identity Study Notes
...
## Contents: Security, Compliance, and Identity Fundamentals
...
- **Microsoft Entra ID** is the current name for Azure Active Directory (Azure AD). Unless a term is explicitly historical or refers to Azure AD B2C, references to Azure AD in the notes below mean Microsoft Entra ID. Existing APIs, URLs, cmdlets, and integrations remain functional, but new content should use Microsoft Entra terminology. See [Microsoft Entra ID naming guidance](https://learn.microsoft.com/en-us/entra/fundamentals/new-name).
- Use phishing-resistant authentication where possible, require multifactor authentication, apply Conditional Access through a least-privilege and Zero Trust design, and use access reviews and Privileged Identity Management to manage privileged access.
- Azure AD B2C is a legacy customer-identity solution and has not been available for new customers since May 1, 2025. Use Microsoft Entra External ID for new customer identity scenarios.
- **Azure Blueprints (Preview)** is being retired on January 31, 2027; phased retirement began July 31, 2026. Use Bicep, Deployment Stacks or Template Specs, Azure Policy, and Azure RBAC for new governance automation. See [Azure Blueprints retirement guidance](https://learn.microsoft.com/en-us/azure/governance/blueprints/blueprint-retirement).

## Describe the concepts of security, compliance, and identity

- The shared responsibility model

    <img src="../../assets/images/3-shared-responsibility-model.png" alt="package" width="450"/>

- Layers of security

    <img src="../../assets/images/4-defense-depth.png" alt="package" width="250"/>

- Confidentiality, Integrity, Availability (CIA)

    <img src="../../assets/images/4-confidentiality-integrity-availability.png" alt="package" width="250"/>

- Zero Trust guiding principles

    <img src="../../assets/images/2-zero-trust-pillars-v2.png" alt="package" width="450"/>
...
- Symmetric and Asymmetric
    - Two top-level types of encryption: symmetric and asymmetric. Symmetric encryption uses the same key to encrypt and decrypt the data. Asymmetric encryption uses a public key and private key pair. Either key can encrypt data, but a single key can’t be used to decrypt encrypted data. To decrypt, you need a paired key. Asymmetric encryption is used for things such accessing sites on the internet using the HTTPS protocol and electronic data signing solutions. Encryption may protect data at rest, or in transit.
        ...
        <img src="../../assets/images/6-encryption.png" alt="package" width="350"/>
...
- Hashing
    ...
    - Hashing is different to encryption in that it doesn't use keys, and the hashed value isn't subsequently decrypted back to the original.
        ...
        <img src="../../assets/images/6-hashing-3-inline.png" alt="package" width="450"/>
...
- With modern authentication, all services, including all authentication services, are supplied by a central identity provider. Information that's used to authenticate the user with the server is stored and managed centrally by the identity provider.
    ...
    - Microsoft Entra ID is Microsoft's cloud-based identity and access management service. It provides identity capabilities for cloud, hybrid, and multicloud application scenarios; Windows Server Active Directory remains the on-premises directory service.
- The concept of Federation
    - Federation enables the access of services across organizational or domain boundaries by establishing trust relationships between the respective domain’s identity provider. With federation, there's no need for a user to maintain a different username and password when accessing resources in other domains.
        ...
        <img src="../../assets/images/5-federated-identification.png" alt="package" width="450"/>
    ...
    - Microsoft Entra is a product family that encompasses identity and network access capabilities, including Microsoft Entra ID.
    - Microsoft Entra ID is Microsoft's cloud-based identity and access management service. Organizations use it to enable employees, guests, workloads, and devices to access the resources they need, including:
        ...
        1. External services, such as Microsoft Office 365, the Azure portal, and any SaaS applications used by your organization.
            ...
            <img src="../../assets/images/azure-active-directory-diagram-expanded.png" alt="package" width="450"/>

    - Microsoft Entra ID offerings include Free, P1, and P2 capabilities. Licensing is assigned through Microsoft Entra and Microsoft 365 offerings; verify current feature eligibility and licensing before implementation.
        1. Microsoft Entra ID Free provides baseline identity and access capabilities.
        1. Microsoft Entra ID P1 adds capabilities such as Conditional Access and advanced identity administration.
        1. Microsoft Entra ID P2 adds advanced identity protection, risk-based policies, and Privileged Identity Management capabilities. Consult current licensing documentation for detailed entitlement and tenant requirements.
    - Microsoft Entra ID identity types
        1. Microsoft Entra ID manages different types of identities: users, service principals, managed identities, and devices.
        1. User: A user identity is a representation of something managed by Microsoft Entra ID. Employees and guests are represented as users in Microsoft Entra ID. If several users have the same access needs, you can create a group and assign permissions to the group instead of individually.
            - Microsoft Entra B2B collaboration, a feature within External ID, includes the capability to add guest users. With B2B collaboration, an organization can securely share applications and services with guest users from another organization.
        1. Service principal: A service principal is an identity for an application. For an application to delegate identity and access functions to Microsoft Entra ID, register the application in Microsoft Entra ID. A service principal is created in each Microsoft Entra tenant where the application is used and enables authentication and authorization to resources secured by that tenant.
            - For service principals to access tenant-secured resources, application developers must manage and protect credentials, or prefer managed identities where available.
        1. Managed identity: **Managed identities are service principals managed by Azure and represented in Microsoft Entra ID.** They eliminate the need for developers to manage application credentials when connecting to supported Azure resources.

            <img src="../../assets/images/when-use-managed-identities-inline.png" alt="package" width="450"/>
        ...
        1. Device: A device is hardware such as a mobile device, laptop, server, or printer. A device identity gives administrators information for access and configuration decisions. Device identities can be set up in different ways in Microsoft Entra ID.
            - Microsoft Entra registered devices support bring-your-own-device and mobile scenarios. A user can access organizational resources using a personal device without using the organizational account as the Windows sign-in account.
            - Microsoft Entra joined devices are joined through an organizational account and are generally organization-owned.
            - Microsoft Entra hybrid joined devices are joined to on-premises Active Directory and Microsoft Entra ID.
            - Device registration and join enable single sign-on to cloud resources. Confirm supported operating systems and join scenarios in current Microsoft Learn documentation before rollout.
    - Microsoft Entra External ID supports external collaboration and customer identity scenarios. Azure AD B2C is legacy and has not been available for purchase by new customers since May 1, 2025.
    ...
    - **Hybrid identities**, Microsoft offers several ways to authenticate.
        1. Microsoft Entra password hash synchronization: Password hash synchronization is the simplest way to enable authentication for on-premises directory objects in Microsoft Entra ID. Users can sign in to cloud services with the same username and password that they use for on-premises Active Directory.
            - Active Directory Domain Services (AD DS) stores a password hash. Microsoft Entra Connect Sync extracts an additional protected hash and synchronizes it to Microsoft Entra ID; the actual password isn't synchronized.

                <img src="../../assets/images/password-hash-sync.png" alt="package" width="450"/>

        1. Microsoft Entra pass-through authentication allows users to sign in to on-premises and cloud applications using the same password. Unlike password hash synchronization, the sign-in agent validates the password against on-premises Active Directory rather than validating it in the cloud.

            <img src="../../assets/images/pass-through-authentication.png" alt="package" width="450"/>

        1. Federated authentication can be appropriate when requirements need capabilities that aren't met by password hash synchronization or pass-through authentication, such as a required third-party authentication solution. Assess operational complexity and resilience before choosing federation.
            - In federated authentication, Microsoft Entra ID hands off authentication to a trusted system, such as on-premises Active Directory Federation Services (AD FS), to validate the user's password.
            - Federated authentication uses Microsoft Entra Connect and additional federation infrastructure, resulting in a larger operational footprint.
            - Organizations that decide to use Federation with Active Directory Federation Services (AD FS), can optionally set up password hash synchronization as a backup in case their AD FS infrastructure fails.
                ...
                <img src="../../assets/images/federated-authentication.png" alt="package" width="450"/>
...
- Authentication methods available in Microsoft Entra ID
    ...
    1. Passwordless authentication
        ...
        - **Microsoft Authenticator app**: As a passwordless authentication method, the Microsoft Authenticator app can be used as a primary sign-in method or as an additional verification option during self-service password reset (SSPR) or Microsoft Entra multifactor authentication events.
...
- Self-service password reset (SSPR)
    - Self-service password reset (SSPR) is a Microsoft Entra ID feature that allows users to change or reset their password without administrator or help desk involvement.
    ...
    - To use self-service password reset, users must be:
        1. Assigned the required Microsoft Entra ID license. Verify the current self-service password reset licensing requirements in Microsoft Learn.
...
- Microsoft Entra Password Protection helps defend against password-spray attacks. Password spraying submits a few commonly used passwords across many accounts to avoid lockout and detection thresholds.
- Hybrid security: administrators can integrate Microsoft Entra Password Protection with on-premises Active Directory. An on-premises component receives the global banned-password list and custom password-protection policies from Microsoft Entra ID.
- Microsoft Entra Conditional Access provides an extra layer of security before allowing authenticated users to access data or other assets. It is implemented through policies that evaluate identity, device, location, application, and risk signals.
- Microsoft Entra built-in and custom roles are a form of RBAC that controls access to Microsoft Entra resources.
...
- **Entitlement management, a Microsoft Entra ID Governance capability, uses access packages to manage access to resources.**
- **Microsoft Entra access reviews** enable organizations to manage group memberships, access to enterprise applications, and role assignments. Regular reviews help ensure that only the right people have access to resources.
    - Access reviews are helpful when:
        1. You have too many users in privileged roles, such as global administrator.
        1. When automation isn't possible, such as when HR data isn't in Microsoft Entra ID.
...
- Privileged Identity Management (PIM) is a Microsoft Entra capability that enables you to manage, control, and monitor privileged access to Microsoft Entra ID, Azure, Microsoft 365, and other supported resources. PIM mitigates the risks of excessive, unnecessary, or misused access permissions.
- **Privileged Identity Management requires appropriate Microsoft Entra ID licensing.**
...
- **Identity Protection requires appropriate Microsoft Entra ID licensing.**
...
- **Microsoft Sentinel** is a scalable, cloud-native SIEM/SOAR solution that delivers intelligent security analytics and threat intelligence across the enterprise.
    ...
    <img src="../../assets/images/3-four-aspects-azure-sentinel.png" alt="package" width="280"/>
...
- **Microsoft 365 Defender** is an enterprise defense suite that protects against sophisticated cyberattacks.
    ...
    <img src="../../assets/images/2-four-aspects-microsoft-365-defender-v2.png" alt="package" width="280"/>

    Microsoft 365 Defender suite protects:

    1. Identities with Microsoft Defender for Identity and Microsoft Entra ID Protection - Microsoft Defender for Identity uses Active Directory signals to identify, detect, and investigate advanced threats, compromised identities, and malicious insider actions directed at your organization.
    ...
    1. Applications with Microsoft Defender for Cloud Apps - Microsoft Defender for Cloud Apps is a comprehensive cross-SaaS solution that brings deep visibility, strong data controls, and enhanced threat protection to your cloud apps.
        ...
        <img src="../../assets/images/mcas-inline.png" alt="package" width="380"/>
    ...
    1. Microsoft 365 Defender portal
        ...
        <img src="../../assets/images/2-defender-portal-inline.png" alt="package" width="450"/>
        ...
        1. Threat analytics
            ...
            <img src="../../assets/images/threat-analytics-inline.png" alt="package" width="450"/>
...
- Microsoft Priva helps you meet these challenges so you can achieve your privacy goals. Priva's capabilities are available through two solutions: **Priva Privacy Risk Management**, which provides visibility into your organization's data and policy templates for reducing risks; and **Priva Subject Rights Requests**, which provides automation and workflow tools for fulfilling data requests.
    ...
    <img src="../../assets/images/priva-overview-inline.png" alt="package" width="450"/>
...
- The **Microsoft Purview compliance portal** brings together all of the tools and data that are needed to help understand and manage an organization’s compliance needs.
    ...
    <img src="../../assets/images/purview-compliance-portal-inline.png" alt="package" width="450"/>
...
- Information protection and data lifecycle management work together to classify, protect, and govern your data where it lives, and wherever it goes.
    ...
    <img src="../../assets/images/2-know-govern-your-data-expanded.png" alt="package" width="450"/>
...
- DLP policies protect content through the enforcement of rules that consist of:
    ...
    <img src="../../assets/images/5-data-loss-prevention-rules.png" alt="package" width="450"/>
    ...
    **Data loss prevention capabilities have been extended to Microsoft Teams chat and channel messages, including messages in private channels.**
    ...
    <img src="../../assets/images/5-data-loss-prevention-policy-tip.png" alt="package" width="350"/>
...
- **Microsoft Purview Insider Risk Management** is a solution that helps minimize internal risks by enabling an organization to detect, investigate, and act on risky and malicious activities. Insider risk management is available in the Microsoft Purview compliance portal.
    - Insider risk management workflow
        ...
        <img src="../../assets/images/2-insider-risk-management-workflow-expanded.png" alt="package" width="450"/>
...
- Microsoft Purview provides three **eDiscovery** solutions: **Content search, eDiscovery (Standard), and eDiscovery (Premium).**
    ...
    <img src="../../assets/images/m365-ediscovery-solution-expanded.png" alt="package" width="450"/>
...
- Microsoft Purview provides two **auditing** solutions: Audit (Standard) and Audit (Premium).
    ...
    <img src="../../assets/images/audit-solutions-expanded.png" alt="package" width="450"/>
...
- Azure Blueprints is in phased retirement and retires January 31, 2027. For new governance automation, use Bicep with Azure Policy, Azure RBAC, Deployment Stacks, and Template Specs. Migrate existing blueprint definitions and assignments before the retirement date.
- The **Microsoft Purview governance portal** provides a unified data governance service that helps you manage your on-premises, multicloud, and software-as-a-service (SaaS) data. The Microsoft Purview governance portal allows you to:
    ...
    1. Empower data consumers to find valuable, trustworthy data.
        ...
        <img src="../../assets/images/microsoft-purview-v2-expanded.png" alt="package" width="550"/>
